...
async function connectExtensionMode(
  ...
  async function stopRecording(name: string): Promise<HarLog> {
    ...
    console.log(
      `[dev-browser] Stopped HAR recording for "${name}" (${har.log.entries.length} entries)`
    );
    return har;
  }

  console.log(`[dev-browser] Extension mode: HTTP RPC to ${serverUrl}`);
  ...
  return {
    async page(name: string, options?: PageOptions): Promise<Page> {
      ...
      if (options?.record !== false && !harPages.has(name)) {
        try {
          ...
          if (harRes.ok) {
            harPages.add(name);
            console.log(`[dev-browser] Auto-started HAR recording for "${name}"`);
          }
        } catch (err) {
          console.warn(
            `[dev-browser] Failed to auto-start HAR recording for "${name}":`,
            err
          );
        }
      }
      ...
      if (isAutoGenerated) {
        try {
          ...
          if (data.closed && data.closed > 0) {
            console.log(
              `[dev-browser] Auto-closed ${data.closed} page(s) for ephemeral session`
            );
          }
        } catch {
    ...
    async selectSnapshotRef(
      ...
      console.warn(
        "[dev-browser] selectSnapshotRef is not available in extension mode. " +
    ...
    async closeAll(): Promise<{ closed: number; pages: string[] }> {
      ...
      if (result.closed > 0) {
        console.log(
          `[dev-browser] Closed ${result.closed} page(s): ${result.pages.join(", ")}`
        );
      }
      harPages.clear();
      return result;
    ...
    async saveArchive(
      ...
      if (!options?.skipHtml) {
        try {
          ...
          console.warn(
            `[dev-browser] Failed to capture rendered HTML for "${name}":`,
            err
          );
        }
      }
      ...
      if (!options?.skipPdf) {
        try {
          ...
          console.warn(
            `[dev-browser] Failed to capture PDF for "${name}":`,
            err
          );
        }
      }
      ...
      console.log(
        `[dev-browser] Saved archive to ${outputPath} (${parts.join(" + ")})`
      );
      return outputPath;
    },
  };
...
export async function connect(
  ...
  if (isEphemeral) {
    console.log(`[dev-browser] Session: ${session} (ephemeral - pages auto-close on disconnect)`);
    ...
    console.log(
      `[dev-browser] Session: ${session} (auto-generated)\n` +
      `[dev-browser] WARNING: Pages will be auto-closed on disconnect since this session is ephemeral.\n` +
      `[dev-browser] For persistent pages across scripts, set CLAUDE_SESSION_ID env var or pass { session: "my-session" } to connect().`
    );
  }
    ...
    connectingPromise = (async () => {
      try {
        if (isElectronMode) {
          ...
          console.log(`[dev-browser] Connecting to Electron CDP at ${cdpUrl}...`);
          ...
          console.log(`[dev-browser] Electron CDP WebSocket: ${wsEndpoint}`);
  ...
  async function getPage(name: string, options?: PageOptions): Promise<PlaywrightPage> {
    ...
    if (isElectronMode) {
      ...
      console.log(`[dev-browser] Electron mode: found ${allPages.length} page(s), ${appPages.length} app page(s)`);
      ...
      console.log(`[dev-browser] Using page: ${page.url()}`);
      return page;
    }
    ...
    if (isExtensionModeServer) {
      ...
      if (allPages.length === 0) {
        console.log(`[dev-browser] No pages visible, forcing reconnect to refresh target list...`);
  ...
  return {
    async page(name: string, options?: PageOptions): Promise<Page> {
      ...
      if (options?.record !== false && !harRecorders.has(name)) {
        try {
          ...
          console.warn(`[dev-browser] Failed to auto-start HAR recording for "${name}":`, err);
        }
      }
    ...
    async close(name: string): Promise<void> {
      ...
      if (isElectronMode) {
        ...
        console.log(`[dev-browser] Electron mode: close("${name}") is a no-op`);
        return;
      }
    ...
    async disconnect(): Promise<void> {
      ...
      if (isAutoGenerated) {
        try {
          ...
          const data = (await res.json()) as { closed?: number };
          if (data.closed && data.closed > 0) {
            console.log(`[dev-browser] Auto-closed ${data.closed} page(s) for ephemeral session`);
          }
        } catch {
    ...
    async closeAll(): Promise<{ closed: number; pages: string[] }> {
      if (isElectronMode) {
        console.log(`[dev-browser] Electron mode: closeAll() is a no-op`);
      ...
      const result = { closed: data.closed ?? 0, pages: data.pages ?? [] };
      if (result.closed > 0) {
        console.log(`[dev-browser] Closed ${result.closed} page(s): ${result.pages.join(", ")}`);
      }
      return result;
    },
    ...
    async saveArchive(
      ...
      if (!options?.skipHtml) {
        try {
          ...
          console.warn(`[dev-browser] Failed to capture rendered HTML for "${name}":`, err);
        }
      }
      ...
      if (!options?.skipPdf) {
        try {
          ...
          console.warn(`[dev-browser] Failed to capture PDF for "${name}":`, err);
        }
      }
      ...
      console.log(`[dev-browser] Saved archive to ${outputPath} (${parts.join(" + ")})`);
      return outputPath;
    },
  };
}
